...
install_bbr() {
	...
	if [[ $test1 == "net.ipv4.tcp_congestion_control = bbr" && $test2 == "net.core.default_qdisc = fq" ]]; then
		...
		_load bbr.sh
		_try_enable_bbr
		[[ ! $enable_bbr ]] && bash <(curl -s -L https://github.com/teddysun/across/raw/main/bbr.sh)
	fi
}
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/srSPEEDiness/V2SPEED/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
...
_help() {
	echo
	echo "........... Informações de ajuda do script de administração do V2Ray por @srSPEEDiness ........."
	echo -e "
